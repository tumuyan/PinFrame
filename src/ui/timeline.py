from PyQt6.QtWidgets import (QTreeWidget, QTreeWidgetItem, QAbstractItemView,
                              QHeaderView, QWidget, QVBoxLayout, QHBoxLayout, QPushButton, QStackedWidget)
from PyQt6.QtCore import Qt, pyqtSignal, QSize, QTimer
from PyQt6.QtGui import QColor, QFont
from i18n.manager import i18n
from ui.timeline_grid_widget import TimelineGridWidget
import os
...
class TimelineWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.list_view_btn = QPushButton(i18n.t("timeline_view_list", "List View"))
        self.list_view_btn.setCheckable(True)
        self.list_view_btn.setChecked(True)
        self.list_view_btn.clicked.connect(lambda: self.switch_to_list_view())
        toolbar.addWidget(self.list_view_btn)

        self.grid_view_btn = QPushButton(i18n.t("timeline_view_grid", "Grid View"))
        self.grid_view_btn.setCheckable(True)
        self.grid_view_btn.clicked.connect(lambda: self.switch_to_grid_view())
    ...
    def _setup_list_widget(self):
        ...
        self.list_widget.dragEnterEvent = self.list_drag_enter_event
        self.list_widget.dragMoveEvent = self.list_drag_move_event
        self.list_widget.dropEvent = self.list_drop_event
    ...
    def set_image_cache(self, cache):
        ...
        self.grid_widget.set_image_cache(cache)

    def set_grid_view_config(self, enabled, width, height, filename_mode):
        """设置网格视图配置"""
        self.grid_view_enabled = enabled
        self.thumbnail_width = width
        self.thumbnail_height = height
        self.filename_line_mode = filename_mode

        # 应用配置到网格视图
        self.grid_widget.set_thumbnail_size(width, height)
        self.grid_widget.set_filename_line_mode(filename_mode)

        # 根据配置切换视图
        if enabled and self.current_view == "list":
            self.switch_to_grid_view()
        elif not enabled and self.current_view == "grid":
            self.switch_to_list_view()

    def get_grid_view_config(self):
        """获取网格视图配置"""
        return {
            "grid_view_enabled": self.grid_view_enabled,
            "thumbnail_width": self.thumbnail_width,
            "thumbnail_height": self.thumbnail_height,
            "filename_line_mode": self.filename_line_mode
        }
    ...
    def refresh_visuals(self):
        ...
        self.grid_widget.update_visuals()

    # Drag and Drop event wrappers for list widget
    def list_drag_enter_event(self, event):
        if event.mimeData().hasUrls():
            event.accept()
        else:
            QTreeWidget.dragEnterEvent(self.list_widget, event)

    def list_drag_move_event(self, event):
        if event.mimeData().hasUrls():
            event.accept()
        else:
            # Prevent dropping ON item (nesting) for internal moves
            # Only allow Above or Below
            target = self.list_widget.itemAt(event.position().toPoint())
            if target:
                ...
                pass
            QTreeWidget.dragMoveEvent(self.list_widget, event)
    ...
    def list_drop_event(self, event):
        if event.mimeData().hasUrls():
            ...
            drop_pos = self.list_widget.dropIndicatorPosition()
            ...
            QTreeWidget.dropEvent(self.list_widget, event)
    ...
    def flatten_tree(self):
        ...
        for parent, children in items_to_move:
            parent_idx = self.list_widget.indexOfTopLevelItem(parent)
            # Insert children after parent
            for offset, child in enumerate(children):
                self.list_widget.insertTopLevelItem(parent_idx + 1 + offset, child)
    ...
    def show_context_menu(self, position):
        ...
        if self.current_view == "list":
            ...
            selected_items = self.grid_widget.get_selected_frames()
        ...
        # Map position to widget coordinates based on current view
        if self.current_view == "list":
            widget = self.list_widget
        else:
            widget = self.grid_widget
    ...
    def add_frame(self, filename, frame_data, orig_width=0, orig_height=0):
        ...
        index = self.list_widget.topLevelItemCount()
        item.setText(0, str(index)) # Initial Index
        ...
        # 添加到网格视图
        self.grid_widget.add_frame(frame_data, index)
    ...
    def _refresh_grid_view(self):
        ...
        for i, frame_data in enumerate(self.frames_data):
            self.grid_widget.add_frame(frame_data, i + 1)
    ...
    # Forward methods to list widget for compatibility
    ...
    def insertTopLevelItem(self, index, item):
        return self.list_widget.insertTopLevelItem(index, item)

    def takeTopLevelItem(self, index):
        return self.list_widget.takeTopLevelItem(index)

    def invisibleRootItem(self):
        return self.list_widget.invisibleRootItem()

    def selectedItems(self):
        if self.current_view == "list":
            return self.list_widget.selectedItems()
        else:
            # 返回模拟的 QTreeWidgetItem 列表（用于兼容性）
            # 这里简化处理，实际使用时应该通过信号传递 frame_data
            return []
