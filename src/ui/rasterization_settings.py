from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel, 
                             QDoubleSpinBox, QPushButton, QCheckBox, QColorDialog)
from PyQt6.QtCore import Qt
from PyQt6.QtGui import QColor
from i18n.manager import i18n

class RasterizationSettingsDialog(QDialog):
    def __init__(self, parent=None, enabled=False, grid_color=None, scale_threshold=2.0):
        super().__init__(parent)
        self.setWindowTitle(i18n.t("dlg_rasterization"))
        self.setFixedSize(350, 220)
        
        layout = QVBoxLayout(self)
        
        # Rasterization Enable Checkbox
        self.enabled_check = QCheckBox(i18n.t("raster_enable"))
        self.enabled_check.setChecked(enabled)
        layout.addWidget(self.enabled_check)
        
        layout.addSpacing(10)
        
        # Grid Color Picker
        color_layout = QHBoxLayout()
        color_layout.addWidget(QLabel(i18n.t("raster_grid_color")))
        
        if grid_color is None:
            grid_color = QColor(0, 0, 0)
        self.grid_color = QColor(grid_color)
        
        self.color_btn = QPushButton()
        self.color_btn.setFixedSize(60, 25)
        self.update_color_button()
        self.color_btn.clicked.connect(self.pick_color)
        color_layout.addWidget(self.color_btn)
        color_layout.addStretch()
        layout.addLayout(color_layout)
        
        # Scale Threshold
        threshold_layout = QHBoxLayout()
        threshold_layout.addWidget(QLabel(i18n.t("raster_scale_threshold")))
        self.threshold_spin = QDoubleSpinBox()
        self.threshold_spin.setRange(1.0, 10.0)
        self.threshold_spin.setSingleStep(0.1)
        self.threshold_spin.setValue(scale_threshold)
        self.threshold_spin.setDecimals(1)
        threshold_layout.addWidget(self.threshold_spin)
        layout.addLayout(threshold_layout)
        
        layout.addSpacing(10)
        
        # Description
        desc_label = QLabel(i18n.t("raster_description"))
        desc_label.setWordWrap(True)
        desc_label.setStyleSheet("color: #999999; font-size: 10px;")
        layout.addWidget(desc_label)
        
        layout.addStretch()
        
        # Buttons
        btn_layout = QHBoxLayout()
        self.ok_btn = QPushButton(i18n.t("btn_ok", "OK"))
        self.ok_btn.clicked.connect(self.accept)
        self.cancel_btn = QPushButton(i18n.t("btn_cancel", "Cancel"))
        self.cancel_btn.clicked.connect(self.reject)
        
        btn_layout.addWidget(self.ok_btn)
        btn_layout.addWidget(self.cancel_btn)
        layout.addLayout(btn_layout)
    
    def pick_color(self):
        color = QColorDialog.getColor(self.grid_color, self, i18n.t("raster_pick_color"))
        if color.isValid():
            self.grid_color = color
            self.update_color_button()
    
    def update_color_button(self):
        self.color_btn.setStyleSheet(
            f"background-color: rgb({self.grid_color.red()}, {self.grid_color.green()}, {self.grid_color.blue()});"
        )
        
    def get_settings(self):
        return {
            "enabled": self.enabled_check.isChecked(),
            "grid_color": self.grid_color,
            "scale_threshold": self.threshold_spin.value()
        }
